...
export default function SearchBar({
  ...
  const searchFunct = () => {
    ...
    if (!trimmed) {
      alert(trimmed + " is not a valid city");
      return;
    }
  ...
  const addToFavorites = () => {
    const trimmed = searchText.trim();
    if (!trimmed) {
      alert("Enter a city before adding to favorites");
      return;
    }

    if (favorites.includes(trimmed)) {
      alert(`"${trimmed}" is already in your favorites!`);
      return;
    }

    setFavorites([...favorites, trimmed]);
    alert(`"${trimmed}" was added to favorites.`);
    setSearchText("");
  };
  ...
  return (
    <section className=" relative z-[50] flex items-center bg-white/20 backdrop-blur-md rounded-full px-4 py-2 w-full max-w-md shadow-lg ">
      ...
      >
        <img src="/assets/hamburger.png" alt="favorites" className="h-4 w-4" />
      </button>
